<script lang="ts">
  ...
  import { addCategory } from "$lib/stores/categories.store";
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      toast.success(`'${trimName}' 카테고리를 추가했습니다.`);
...
<Dialog.Root bind:open>
  ...
  >
    ...
    <div class="underline underline-offset-2">새 카테고리 추가</div>
  </Dialog.Trigger>
  <Dialog.Content class="sm:max-w-[425px]">
    <Dialog.Header>
      <Dialog.Title>새 카테고리 추가</Dialog.Title>
    </Dialog.Header>
    <Input
      id="name"
